...
open class ImageSlideshow: UIView {
    ...
    fileprivate func loadImages(for scrollViewPage: Int) {
    ...
    fileprivate func setTimerIfNeeded() {
        if slideshowInterval > 0 && scrollViewImages.count > 1 && slideshowTimer == nil {
            DispatchQueue.main.async {
                self.slideshowTimer = Timer.scheduledTimer(timeInterval: self.slideshowInterval, target: self, selector: #selector(ImageSlideshow.slideshowTick(_:)), userInfo: nil, repeats: true)
            }
        }
    }
    ...
    @objc fileprivate func pageControlValueChanged() {
